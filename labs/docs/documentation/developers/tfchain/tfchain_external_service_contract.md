...
- Both service and consumer need to have their respective twin created on TFChain
...
Make sure to use right links depending on the targeted network.
...
If twin IDs are wrong on service contract fields it means the contract does not correspond to the last created contract.
